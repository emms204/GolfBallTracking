...
Windows:
```batch
build\calibration\Release\calibrate_camera.exe --input 0 --pattern_size 9x6 --square_size 20 --output camera_calibration.yaml
```
...
Windows:
```batch
build\calibration\Release\calibrate_camera.exe --input C:/path/to/images --pattern_size 9x6 --square_size 20 --output camera_calibration.yaml
```
...
Windows:
```batch
build\src\Release\detection_test_app.exe --video=your_video.mp4 --model=best.onnx --classes=classes.txt --calibration=your_calibration.yaml
```
