...
# Debug - Check image directories
echo -e "${YELLOW}DEBUG: Checking image directories...${NC}"
ls -la "$MASTER_IMAGES_DIR" | head -n 10
echo "..."
ls -la "$SLAVE_IMAGES_DIR" | head -n 10
echo "..."
...
# Create calibration script in our local directory
echo -e "${YELLOW}Creating calibration script at $PYTHON_CALIB_SCRIPT${NC}"

cat > "$PYTHON_CALIB_SCRIPT" << 'EOF'
import numpy as np
import cv2
import glob
import os
...
$PYTHON_CMD "$PYTHON_CALIB_SCRIPT" --master "$MASTER_IMAGES_DIR" --slave "$SLAVE_IMAGES_DIR" --output "$OUTPUT_DIR"
